...
*/
//
//ќгромное спасибо ¬адиму aka Installero за помощь с хтмл.
//
//
//
//
...
function getmapname($map, $servernum)
{
        ...
        //type 6 'А' Mount
        //type 7 '€0' In and out
        //type 8 'Ш' Lord of the rings
        //type 9 'ƒР' Three sides
        //type 10 '' Dark lands
        //type 11 '' Grave
        //type 12 '' Rodion
/*        if(strpos($clean[0], '') > 0) { $clean = explode('', "$dirty"); } //type 2
        ...
        if(strpos($clean[0], 'Ґ') > 0) { $clean = explode('Ґ', "$dirty"); } //type 5
        if(strpos($clean[0], 'А') > 0) { $clean = explode('А', "$dirty"); } //type 6
        if(strpos($clean[0], '€0') > 0) { $clean = explode('€0', "$dirty"); } //type 7
        if(strpos($clean[0], 'Ш') > 0) { $clean = explode('Ш', "$dirty"); } //type 8
        if(strpos($clean[0], 'ƒР') > 0) { $clean = explode('ƒР', "$dirty"); } //type 9
...
*/
        ...
        for($i = 0; $b > $i; $i++)
        {
                $ord = ord($name[$i]);
                //
                if ($ord == '32' || $ord == '46') { } //“очка или пробел
                elseif ($ord > '32' && $ord < '63') { } //÷ифра и знаки типа ; ...
...
foreach($hat as $line)
{
        ...
        {
                //¬ыполн€ютьс€ действи€ если така€ строка найдена.
                $clean = explode('=', $line);
...
<TR align=middle>
                ...
                <TD align=middle width=244><STRONG>Ќазвание карты</STRONG></TD>
                <TD align=middle width=128><STRONG>¬рем€ (в
              часах)</STRONG></TD></TR>
';
fwrite($fp, $output);
$aggr = 0;
...
for($ii = 0; $manymaps > $ii; $ii++)
{
        ...
        fwrite($fp, $output);
        $output = $server['server'.$i]['maps'][$keyz[$mapi]]['name'];
