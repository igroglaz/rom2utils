...
{
        $mapspath = 'D:\Games\Allods\server'."$servernum".'\maps\\';
        $maps = "$mapspath"."$map";
        $name = '';
        global  $name;
...
for( $ServerNum = 1; $ServerNum <= $ServerCount ; $ServerNum++ ) {
	...
	for( $ii = 0; $manymaps > $ii; $ii++) {
		$output = '<map><mapurl>';
		fwrite($fp, $output);
		$output = $pathto.$Servers['server'.$ServerNum]['maps'][$keyz[$mapi]]['alm'].".7z'";
		if($output == '') {
