...
%disable=(
#"7164"=>0, #"Very Rare Meteoric Long Sword", #квест средка Stone Curse Spell Power 30
#"716A"=>0, #"Very Rare Meteoric Morning Star", #квест средка
#"716F"=>0, #"Very Rare Meteoric Pike", #квест средка
#"7172"=>0, #"Very Rare Meteoric Axe", #квест средка
...
"0E41"=>5, #"Potion Body",
"0E43"=>5, #"Potion Mind",
"0E42"=>5, #"Potion Reaction",
"0E44"=>5, #"Potion Spirit",
#"0E54"=>0, #"Quest Amulet",
#"0E4E"=>0, #"Quest Banner",
#"0E4F"=>0, #"Quest Crown1",
...
"716C"=>0, #"Very Rare Meteoric War Hammer"
 ...
 @sps_disable = ( #сила заклов в вещах 0-100
  ...
  31,#"Stone Curse",           # каменное проклятие
...
#	if ($exp_astral_shooting>1600) {$enable=$enable+1; }
	if ($money>300000000) {$enable=$enable+1; } 
	...
	if ($spirit_level>44) {$enable=$enable+1; }
 	if ($spirit_level+$body_level+$reaction_level+$mind_level>160) {$enable=$enable+1; }
	foreach $i (@sps) 
	{
		if ($spells & (1 << $idxx)) 
		{
...
$log_dir = "C:\\Allods2\\plscript\\serv7_log\\";
