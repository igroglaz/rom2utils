...
%effects=();
	...
	if (($b0 & 0x80) == 0x80) {
	    $cc = $b0-0x80;
	    $count=$cc;
#	    print F "  found simple object: ID=$IDh, count=$cc\n";
	} elsif (($b0 & 0x20) == 0x20) {
	    $price = readil($idx); $idx+=4;
	    $ccc = $b0 & 0xF;
#	    print(F "  found magic object: ID=$IDh, price=$price gold\n");
	    $i = 0;
	    $count=1;	
	    while ($i++ < $ccc) {
		$eid = readbl($idx); $idx++;
		...
		$evl = readbl($idx); $idx++;
#		print(F "   effect ID=$eIDh, value=$evl\n");
		$effects{$eIDh}=$evl;
...
#	    print(F "  found simple object: ID=$IDh, count=$ccc\n");
	    $count=$ccc;
	}
...
#	print(F "  on-the-body:\n");
	...
	$idxx = 0;
#	print(F "Body: $body_level\n");
	if ($body_level>44) {$enable=$enable+1; 
#print F "enable=$enable\n";
		}
#	print(F "Reaction: $reaction_level\n");
	if ($reaction_level>44) {$enable=$enable+1; 
#print F "enable=$enable\n";
		}
#	print(F "Mind: $mind_level\n");
	if ($mind_level>44) {$enable=$enable+1; 
#print F "enable=$enable\n";
		}
#	print(F "Spirit: $spirit_level\n");
...
#	print(F "   fire/blade exp: $exp_fire_blade\n");
#	print(F "   water/axe exp: $exp_water_axe\n");
#	print(F "   air/bludgeon exp: $exp_air_bludgeon\n");
#	print(F "   earth/pike exp: $exp_earth_pike\n");
#	print(F "   astral/shooting exp: $exp_astral_shooting\n");
    }
    else {
