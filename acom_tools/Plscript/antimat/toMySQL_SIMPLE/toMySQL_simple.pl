...
#					print "\n";
                                        if ($loginall[$num_of_serv]{"$login"} != 0)
					  {
						#$sth = $dbh->Query("call InsertDATA(\'".$login."\',".$loginall[$num_of_serv]{"$login"}.",".$loginmat[$num_of_serv]{"$login"}.",".$login_ochki[$num_of_serv]{"$login"}.");") or die $SimpleMySQL::db_errstr;
                                                $dbh->do("call ".$database.".InsertDATA(\'".$login."\',".$loginall[$num_of_serv]{"$login"}.",".$loginmat[$num_of_serv]{"$login"}.",".$login_ochki[$num_of_serv]{"$login"}.");");
						#$sth = dbselect("call InsertDATA(\'".$login."\',".$loginall[$num_of_serv]{"$login"}.",".$loginmat[$num_of_serv]{"$login"}.",".$login_ochki[$num_of_serv]{"$login"}.");");
						#$sth = dbselect("call InsertDATA(\'".$login."\',".$loginall[$num_of_serv]{"$login"}.",".$loginmat[$num_of_serv]{"$login"}.",".$login_ochki[$num_of_serv]{"$login"}.");");
					  }
	...
	#			print $login." ".$loginall[$num_of_serv]{"$login"}." ".$loginmat[$num_of_serv]{"$login"}." ".$login_ochki[$num_of_serv]{"$login"}."\n";
				#$sth = $dbh->Query("call InsertDATA(\'".$login."\',".$loginall[$num_of_serv]{"$login"}.",".$loginmat[$num_of_serv]{"$login"}.",".$login_ochki[$num_of_serv]{"$login"}.");") or die $SimpleMySQL::db_errstr;
				$sth = $dbh->do("call ".$database.".InsertDATA(\'".$login."\',".$loginall[$num_of_serv]{"$login"}.",".$loginmat[$num_of_serv]{"$login"}.",".$login_ochki[$num_of_serv]{"$login"}.");");
			  }
			delete $player2login[$num_of_serv]{"$name"};
			delete $login2player[$num_of_serv]{"$login"};
		  }
