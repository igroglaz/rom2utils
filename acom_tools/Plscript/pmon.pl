...
my @arr;
my $keysec;
my @levels=("","Easy","Medium","Hard","Horror");
my %type=();
$type{"COOPERATIVE"}="Cooperative";
$type{"ARENA"}="Arena";
$type{"TEAMPLAY"}="Teamplay";
my %visprt=();
$visprt{0}="Visible";
$visprt{1}="Invisible";
...
open(INI,"C:\\Allods2\\PLScript\\status.ini");
$hat_cfg_name=<INI>;
chomp $hat_cfg_name;
close(INI);
#print $hat_cfg_name . "\n";
  ...
  while(<HAT_CFG>)
   {
	...
	if(/chr_base=(.*)\\?/)
        	  {
	  $logged_in="C:\\Allods2\\" . $1 . "\\logged_in\\";
	 }
#	if(/server=\d+\.\d+\.\d+\.\d+:\d+,(.+),(.*)/) 
	if(/server=.+:\d+,(.+),(.*)/) 
	  {
...
open(OUT_ADM,">C:\\wamp\\www\\api\\status.xml");   
print OUT_ADM "<?xml version=\"1.0\" encoding=\"Windows-1251\"?>";
print OUT_ADM "<status>";

#print "C:\\Allods2\\PLScript\\scan.ini\n";
$servern=0;
open(SERVLIST, "C:\\Allods2\\PLScript\\scan.ini");
$section="";
while(<SERVLIST>)
 { 
   $servern=$servern+1;
   ...
   while(<LISTING>)
    {
	  ...
	  $lgn=$_;
	  $lgn_file=$name . $_;
	  open(C, $lgn_file) || die "can't open $_\n";
	  binmode C;
...
#    }
	...
	print  OUT_ADM $#tmp+1;
        ...
        if($#tmp>-1) 
        {
            ...
            {
################################################            
#эти строчки добавил ЛАС. убрал баг!!!!
##$pey=$key; #сохраняю копию переменной
##$key=substr($key,(rindex($key,"\\"))+1); #изменяю оригинал  
##################################################
$lin_name=$logged_in . $key;
            ...
            foreach my $SymbLine (@HTMLSymbReal) 
            {
		($ascii, $html) = split(/[\s\t]+/, $SymbLine);
		$value =~ s/$ascii/$html/g;
		print "Replacing " . $ascii . " -> " . $html . "\n";
	    }
	    $value =~ s/\s/&#160;/g;
                ...
                print OUT_ADM "http://hat.allods2.com/status/show.php?a2c=".$name.$key;
...
###################################
###$key=$pey; #восстанавливаю оригинал из копии
### fpizdu govnokod
###################################
