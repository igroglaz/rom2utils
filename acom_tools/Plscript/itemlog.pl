...
open(INI,"/home/allods2/Documents/Allods2/scripts/status.ini");
$hat_cfg_name=<INI>;
chomp $hat_cfg_name;
close(INI);
#print $hat_cfg_name . "\n";
  ...
  while(<HAT_CFG>)
   {
	...
	if(/chr_base=(.*)\\?/)
        	  {
	  $logged_in="/home/allods2/Documents/Allods2/" . $1 . "/logged_in/";
	 }
	if(/server=\d+\.\d+\.\d+\.\d+:\d+,(.+),(.*)/) 
	  {
	    $server_list{$1}=$2;
...
#open(OUT_STD,">>C:\\Allods2\\logs\\itemlog.log");   
...
while(<SERVLIST>)
 {
   ...
   chomp;
   $name=$_;
   open(OUT_STD,">>/home/allods2/Documents/Allods2/logs/itemlog_$servern.log");   
   open(LISTING,"ls -1 $name|");
   $name=~s/\*\.$//;
   $name=~/server(\d+)/i;
   $server_id=$1;
...
#	print("  memory:\n");
	...
	$body_t+=$body_pack;
	$mind_t+=$mind_pack;
	$reaction_t+=$reaction_pack;
	$spirit_t+=$spirit_pack;
	$res_total=$res_total+$res;
	print OUT_STD  "\n";
    }
...
#
	...
	$body_t+=$body_level;
	$mind_t+=$mind_level;
	$reaction_t+=$reaction_level;
	$spirit_t+=$spirit_level;
	print OUT_STD "spells: ";
	foreach $i (@sps) {
...
print OUT_STD "BRMS: $body_t $reaction_t $mind_t $spirit_t\n";
